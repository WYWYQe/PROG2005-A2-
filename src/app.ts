// Category type
type ItemCategory = 'Electronics' | 'Furniture' | 'Clothing' | 'Tools' | 'Other';

// Stock status type
type StockStatus = 'In Stock' | 'Low Stock' | 'Out of Stock';

// Popular flag
type PopularFlag = 'Yes' | 'No';

// Current list view mode
type ViewMode = 'all' | 'popular' | 'search';
...
// Dropdown options
const CATEGORIES: ItemCategory[] = ['Electronics', 'Furniture', 'Clothing', 'Tools', 'Other'];
const STOCK: StockStatus[] = ['In Stock', 'Low Stock', 'Out of Stock'];
const POPULAR: PopularFlag[] = ['Yes', 'No'];
...
const usedIds = new Set<string>(inventoryList.map((i) => i.id));

// Page state
let viewMode: ViewMode = 'all'; // current view mode
let searchQuery = ''; // current search keyword
let editingName: string | null = null; // current editing item name
let pendingDeleteName: string | null = null; // item name pending deletion
...
function findByName(name: string): InventoryItem | undefined {
...
function getDisplayedItems(): InventoryItem[] {
  ...
  if (viewMode === 'popular') return inventoryList.filter((i) => i.popular === 'Yes');
...
function renderTableRows(items: InventoryItem[]): string {
  if (items.length === 0) return `<tr><td colspan="10" class="empty">No data</td></tr>`;
...
function readForm() {
  return {
    id: ((document.getElementById('field-id') as HTMLInputElement)?.value ?? '').trim(),
    name: ((document.getElementById('field-name') as HTMLInputElement)?.value ?? '').trim(),
    category: (document.getElementById('field-category') as HTMLSelectElement)?.value as ItemCategory,
    quantity: Number((document.getElementById('field-qty') as HTMLInputElement)?.value),
    price: Number((document.getElementById('field-price') as HTMLInputElement)?.value),
    supplierName: ((document.getElementById('field-supplier') as HTMLInputElement)?.value ?? '').trim(),
    stockStatus: (document.getElementById('field-stock') as HTMLSelectElement)?.value as StockStatus,
    popular: (document.getElementById('field-popular') as HTMLSelectElement)?.value as PopularFlag,
    notes: ((document.getElementById('field-notes') as HTMLInputElement)?.value ?? '').trim(),
  };
}

// Form validation
function validateForm(f: ReturnType<typeof readForm>): boolean {
  if (!f.id) return showMessage('Item ID is required', 'err'), false;
  if (!f.name) return showMessage('Item name is required', 'err'), false;
  if (!CATEGORIES.includes(f.category)) return showMessage('Invalid category', 'err'), false;
  if (!Number.isFinite(f.quantity) || f.quantity < 0) return showMessage('Quantity must be a non-negative number', 'err'), false;
  if (!Number.isFinite(f.price) || f.price < 0) return showMessage('Price must be a non-negative number', 'err'), false;
  if (!f.supplierName) return showMessage('Supplier is required', 'err'), false;
  if (!STOCK.includes(f.stockStatus)) return showMessage('Invalid stock status', 'err'), false;
  if (!POPULAR.includes(f.popular)) return showMessage('Invalid popular value', 'err'), false;
  return true;
}
...
function bindEvents(): void {
  ...
  document.getElementById('btn-modal-confirm')?.addEventListener('click', () => {
    ...
    if (idx === -1) {
      ...
      if (editingName === removed.name) editingName = null;
...
function onSave(): void {
  ...
  if (editingName) {
    ...
    if (idx === -1) return showMessage('Item not found', 'err');

    // Updated name must not conflict with others
    const conflict = inventoryList.find((i, j) => j !== idx && i.name === f.name);
    if (conflict) return showMessage('Updated name conflicts with another item', 'err');
  ...
  if (findByName(f.name)) return showMessage('Item name already exists, please use another name', 'err');

  // Add mode: ID must be unique
  if (usedIds.has(f.id)) return showMessage('Item ID already exists, please use another ID', 'err');
